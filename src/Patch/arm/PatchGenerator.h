    ...
    RelocatableInst::SharedPtrVec generate(const llvm::MCInst *inst, rword address, rword instSize,
        ...
        if(type == ConstantType) {
            ...
            if (inst->getOperand(op).isImm()) {
              ...
              //std::cout << "pcoffset:" << std::hex << std::showbase << pcoffset << std::endl;
              //std::cout << "imm:" << std::dec << std::showbase << imm << std::endl;
              int64_t value = address + pcoffset + imm;
              //std::cout << "value: " << std::hex << std::showbase << value << std::endl;
              //if (cpuMode == CPUMode::Thumb) {
              //  value += value % 4;
              //}
              RelocatableInst::SharedPtrVec rinst = {
                Ldr(
                    cpuMode,
