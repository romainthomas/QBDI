...
ExecBlock::ExecBlock(Assembly* assembly[CPUMode::COUNT], VMInstanceRef vminstance) : vminstance{vminstance} {
    ...
    // Copy Assembly
    for(size_t i = 0; i < CPUMode::COUNT; i++) {
    ...
    context = reinterpret_cast<Context*>(dataBlock.base());
    shadows = reinterpret_cast<rword*>(reinterpret_cast<uintptr_t>(dataBlock.base()) + sizeof(Context));
    ...
    // JIT Epilogue
    LogDebug("ExecBlock::Contructor", "JIT Epilogue");
    ...
    // JIT Prologue
    LogDebug("ExecBlock::Contructor", "JIT Prologue");
    codeStream->seek(0);
    ...
    // It results of empty instructions between Prologue and first instruction
...
void ExecBlock::show() const {
    ...
    for (size_t i = 0; i < jitCode.size(); i += instSize)
    {
        llvm::MCInst inst;
        std::string disass;
        uintptr_t address = reinterpret_cast<uintptr_t>(codeBlock.base()) + i;
        ...
        assbly->printDisasm(inst, disassOs);
        disassOs.flush();
        fprintf(stderr, "0x%" PRIRWORD " %s\n", address, disass.c_str());
    }

    fprintf(stderr, "---- CONTEXT ----\n");
    for (size_t i = 0; i < NUM_GPR; ++i) {
        fprintf(stderr, "%s=0x%016" PRIRWORD " ", assbly->getRegisterName(GPR_ID[i]), QBDI_GPR_GET(&context->gprState, i));
        if(i % 4 == 0) fprintf(stderr, "\n");
    }
    fprintf(stderr, "\n");

    fprintf(stderr, "---- SHADOWS ----\n[");
    for (size_t i = 0; i + 1 < shadowIdx; ++i) {
        fprintf(stderr, "0x%016" PRIRWORD ", ", shadows[i]);
    }
    if(shadowIdx > 0) {
      fprintf(stderr, "0x%016" PRIRWORD, shadows[shadowIdx - 1]);
    }
    fprintf(stderr, "]\n");
}
...
VMAction ExecBlock::execute() {
    ...
    do {
        ...
        // Host -> Guest (Instrumented)
        // ============================
        {
          run();
        }
        // ===========================
        // Guest did a "BreakHost"
        ...
        if (context->hostState.callback != 0) {
            ...
            auto&& userCallback = reinterpret_cast<InstCallback>(context->hostState.callback);
            VMAction action = userCallback(
                vminstance,
                &context->gprState, &context->fprState,
                reinterpret_cast<void*>(context->hostState.data)
            );
...
SeqWriteResult ExecBlock::writeSequence(std::vector<Patch>::const_iterator seqIt, std::vector<Patch>::const_iterator seqEnd, SeqType seqType) {
    ...
    uint16_t seqID        = static_cast<uint16_t>(getNextSeqID());
